...
import (
	...
	"github.com/gzjjjfree/gzv2ray/common/log"
	"github.com/gzjjjfree/gzv2ray/serial"
)
...
func (err *Error) WithPathObj(obj interface{}) *Error { //方法 WithPathObj 设置 Error.pathObj 为 参数 obj
	err.pathObj = obj
	return err
}
...
func (err *Error) pkgPath() string {
	...
	path := reflect.TypeOf(err.pathObj).PkgPath() //PkgPath 返回已定义类型的包路径，即唯一标识包的导入路径，例如“encoding/base64”。
	//如果类型是预声明的（string、error）或未定义（*T、struct{}、[]int 或 A，其中 A 是未定义类型的别名），则包路径将为空字符串。
	path = strings.TrimPrefix(path, "github.com/gzjjjfree/gzv2ray/")
	path = strings.TrimPrefix(path, "github.com/gzjjjfree/gzv2ray")
	return path
}
